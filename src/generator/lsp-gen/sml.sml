...
structure SML =
  struct
    ...
    and dec
      ...
      | STRdec of id * (bool * sigexp) option * strexp
      | OPENdec of id list
  ...
  (* construct a simple function declaration of the form `fun f (x1, ..., xn) = e` *)
    fun simpleDec (f, xs, e) = FUNdec([], [simpleFB (f, xs, e)])
  ...
  (* construct a simple let binding of the form `let val x = e1 in e2` *)
    ...
    fun conTy id = CONty([], id)

    (* the pervasive types *)
    val unitTy = CONty([], "unit")
    val boolTy = CONty([], "bool")
    val charTy = CONty([], "char")
    val intTy = CONty([], "int")
    val wordTy = CONty([], "word")
    val realTy = CONty([], "real")
    val stringTy = CONty([], "string")
    val substringTy = CONty([], "substring")
    val exnTy = CONty([], "exn")
    val orderTy = CONty([], "order")

    (* pervasive type constructors *)
    fun optionTyc ty = CONty([ty], "option")
    fun listTyc ty = CONty([ty], "list")
    fun refTyc ty = CONty([ty], "ref")
    fun arrayTyc ty = CONty([ty], "array")
    fun vectorTyc ty = CONty([ty], "vector")
